...
def merge_tiles(tile_files, output_path):
    ...
    # Build VRT (Virtual Dataset) then translate to GeoTIFF
    vrt_path = output_path.parent / (output_path.stem + '.vrt')
    ...
    gdal.Translate(str(output_path), str(vrt_path), format='GTiff', creationOptions=['COMPRESS=LZW'])

    # Clean up VRT and tiles
    vrt_path.unlink()
...
def download_rwanda_data():
    ...
    # Rwanda bounds
    rwanda_bounds = [28.85, -2.85, 30.90, -1.05]  # [min_lon, min_lat, max_lon, max_lat]

    # Create grid (adjust these numbers to stay under 50MB per tile)
    # Rwanda is ~26,000 km² - we'll use 4x4 grid = 16 tiles
    print("Creating tile grid (4x4 = 16 tiles)...")
...
def download_kigali_only():
    ...
    # Kigali bounds (much smaller area)
...
if __name__ == "__main__":
    ...
    print()
    print("2. Download full Rwanda in tiles (SLOW - 20-40 minutes, ~100-200MB)")
    ...
    if choice == "1":
        ...
        print("⚠️  This will take 20-40 minutes and make ~32 API calls")
        print("   (16 tiles × 2 datasets)")
